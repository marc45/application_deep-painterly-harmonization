...
def vis_one_image_opencv(
        ...
        return vis_one_image_opencv___bak(im, boxes, segms, keypoints, thresh, kp_thresh,
                show_box, dataset, show_class)

def vis_one_image_opencv_mask_list(
        im, boxes, segms=None, keypoints=None, thresh=0.9, kp_thresh=2,
        show_box=False, dataset=None, show_class=False):
    ...
    for i in sorted_inds:
        ...
        if segms is not None and len(segms) > i:
            mask_bb = np.zeros(im.shape,dtype=np.uint8)
            color_mask = color_list[mask_color_id % len(color_list), 0:3]
            mask_color_id += 1
            mask_bb = vis_mask(mask_bb, masks[..., i], color_mask)
            mask_cc = mask_bb[:,:,0]
            mask_cc[mask_cc>0]=255
            mask_collect.append(mask_cc)
...
def vis_one_image_opencv___bak(
        im, boxes, segms=None, keypoints=None, thresh=0.9, kp_thresh=2,
        show_box=False, dataset=None, show_class=False):
    """Constructs a numpy array with the detections visualized."""
